...
func LeaderboardGET(md common.MethodData) common.CodeMessager {
	...
	query := fmt.Sprintf(lbUserQuery+` ORDER BY users_stats.ranked_score_%[1]s DESC, users_stats.ranked_score_%[1]s DESC`, m)
	if common.Int(md.Query("rx")) == 1 {
		query = fmt.Sprintf(rxUserQuery+` ORDER BY rx_stats.ranked_score_%[1]s DESC, rx_stats.ranked_score_%[1]s DESC`, m)
	}
	if common.Int(md.Query("rx")) == 2 {
